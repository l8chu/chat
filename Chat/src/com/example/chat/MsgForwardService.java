...
public class MsgForwardService {
    ...
    public MsgForwardService(Context context1, Handler handler1) {
    		...
    		try {
    			...
    			while (true) { 
	    			...
	    			if ((myLocalIP.trim()).equals(DestIP.trim())){	
		    			...
		    			.sendToTarget();
    					Log.log("Accepting [RC] packet from:"+ remoteIP.getHostAddress()+
    							"packet contents:["+s+"]");
	    				android.util.Log.i(TAG1,"point 5");
			    			...
			    			.sendToTarget();
	    					Log.log("Accepting [RA] packet from:"+ remoteIP.getHostAddress()+
	    							"packet contents:["+s+"]");
	    				}
	    				else if (MsgType.equalsIgnoreCase("CM")){
	    					//fHandler.obtainMessage(OnlineUsers.CHAT_MESSAGE, SourceIP)
			    			//.sendToTarget();
	    					Log.log("Accepting [CM] packet from:"+ remoteIP.getHostAddress()+
	    							"packet contents:["+s+"]");
	    				}
	    				else if (MsgType.equalsIgnoreCase("RB")){
	    					fHandler.obtainMessage(OnlineUsers.REPLY_BUSY,SourceIP).sendToTarget();
	    					
	    				}
	    			}
	    			else if(!((myLocalIP.trim()).equals(DestIP.trim()))){ // FORWARD MESSAGE TO DEST IP
	    				if (MsgType.equalsIgnoreCase("RC")){
	    					OnlineUsers.AddtoQueue(s);
	    					...
	    					Log.log("Forwarding [RB] packet from:"+ remoteIP.getHostAddress()+
	    							"packet contents:["+s+"]");
	    					OnlineUsers.AddtoQueue(s);
	    				}
	    			}
